<?php
/**
 * vote1_callback.php — Callback Maishapay pour vote1 (Carte Visa/Mastercard uniquement)
 * Mobile Money = Unipesa (géré dans vote1_api.php raw callback)
 * 
...
try{
    ...
    if($existing){
        ...
        if($isBrowser || isset($getParams['ref']) || strpos($requestUri, 'ref=')!==false){
            ...
            $redirectUrl = $scheme.'://'.$host.'/vote1.php?candidat='.(int)$candidate_id.'&concours_id='.(int)$concours_id.'&etape_id='.($etape_id? (int)$etape_id : '').'&receipt='.urlencode($reference).'&status='.urlencode($internalStatus).'&method=card';
