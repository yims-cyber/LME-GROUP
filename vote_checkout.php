<?php
// vote_checkout.php — Redirection sécurisée vers MaishaPay / CyberSource pour carte
// FIX: Cancel Order sur https://secureacceptance.cybersource.com allait vers https://www.arakapay.com/ au lieu de vote.php
// Solution: page intermédiaire sans auto-redirect + polling vers vote_api.php + bouton Annuler qui marque echoue + retour vote.php
// + ouvre paiement dans nouvel onglet depuis vote.php pour garder onglet principal ouvert
...
try{
    ...
    $voteBase = $scheme.'://'.$host.'/vote.php?candidat='.(int)$tx['participante_id'].'&concours_id='.(int)$tx['concours_id'].'&etape_id='.($tx['etape_id']? (int)$tx['etape_id']:'').'&receipt='.urlencode($ref);
...
<div class="card">
  ...
  <p class="small">⚠️ Si vous cliquez "Cancel Order" sur <b>secureacceptance.cybersource.com</b>, vous atterrirez sur <b>arakapay.com</b> (config CyberSource drc_proxypay). Revenez sur cet onglet et cliquez "Annuler et retourner" pour revenir dans vote.php avec la vraie raison (solde insuffisant etc).</p>
  <div class="actions">
    ...
    <a href="<?= htmlspecialchars($cancelUrl) ?>" class="btn btn-outline">❌ Annuler et retourner à vote.php</a>
    <a href="<?= htmlspecialchars($voteBase.'&status=en_attente') ?>" class="btn btn-outline">↩ Retour à vote.php (sans annuler)</a>
  </div>
